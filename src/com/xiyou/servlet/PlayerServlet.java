...
public final class PlayerServlet extends HttpServlet implements Servlet{
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		if("isPlayerIdExist".equals(method)){
			...
			choose(request,response,session);
		}
	}
	/**
	 * 判断账号是否存在
	...
	public void choose(HttpServletRequest request, HttpServletResponse response,HttpSession session){
		String o1 = request.getParameter("optradio");
		String o2 = request.getParameter("style");
		...
		try {
			response.sendRedirect("pages/game.jsp");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
